...
// 檢查是否有教師發起的模擬測驗
let preview = false;
const simUnitId = localStorage.getItem('sport115-teacher-sim');
if (simUnitId) {
  preview = true;
}
...
const pre=()=>preview && simUnitId? '<div class="notice" style="background:#dbeafe;color:#1e40af;display:flex;justify-content:space-between;align-items:center;"><strong>教師模擬測驗中</strong> <button id="exitSim" style="padding:4px 12px;cursor:pointer;">結束模擬並返回教師後台</button></div>' : (preview?'<div class="notice"><strong>教師預覽・不可繳交</strong><br>可提前檢查所有單元。</div>':'');
...
function wireHome(){
  document.getElementById('home')?.addEventListener('click',()=>{
    if(simUnitId){ localStorage.removeItem('sport115-teacher-sim'); location.hash='#teacher'; location.reload(); return; }
    save();home();
  });
  document.getElementById('exitSim')?.addEventListener('click',()=>{
    localStorage.removeItem('sport115-teacher-sim');
...
function home(){
  if(simUnitId){
    const targetId = +simUnitId;
    if(targetId === 1){ orientation(); return; }
    const u = C.units.find(x=>x.id===targetId);
    if(u){ unit=u; study(); return; }
  }

  screen='home';unit=null;session=null;clearTimeout(timer);
  ...
  const isExam=!u.id,open=preview||E.isOpen(u);return `<article class="week ${isExam?'exam':open?'open':''}"><div class="row between"><span class="eyebrow">第${u.week}週 · ${roc(u.date)}</span><span class="pill">${isExam?'考試週':open?'可開始測驗':'尚未開放'}</span></div><h3>${u.id?'單元'+u.id+'：':''}${esc(u.title)}</h3><p>${isExam?'依老師安排，無新增英文題庫。':u.week===1?'熟悉操作與課堂規則，不列入正式成績。':esc(u.grammar)}</p>${!isExam?`<div class="row"><button class="${open?'':'secondary'}" data-week="${u.week}">${u.week===1?(open?'查看導覽':'尚未開放'):(open?'進入單元':'尚未開放')}</button></div>${!open?`<p>將於 ${roc(u.date)} 13:00 開放</p>`:''}`:''}</article>`;}).join('')}</div>`;
  
  app.querySelectorAll('[data-week]').forEach(b=>b.onclick=()=>{
    const week=+b.dataset.week;
    const u=records.find(x=>x.week===week);
    if(!preview&&!E.isOpen(u))return;
    if(week===1){orientation();}
    else{unit=C.units.find(x=>x.week===week);study();}
    top();
  });
}
